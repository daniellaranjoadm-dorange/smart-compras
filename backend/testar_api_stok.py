...
try:
    data = resp.json()
    print("TIPO_JSON:", type(data).__name__)
    if isinstance(data, dict):
        print("CHAVES_TOPO:", list(data.keys())[:30])
    elif isinstance(data, list):
        print("TAMANHO_LISTA:", len(data))
        if data:
            print("TIPO_ITEM_0:", type(data[0]).__name__)
            if isinstance(data[0], dict):
                print("CHAVES_ITEM_0:", list(data[0].keys())[:40])

    with open("stok_api_sample.json", "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print("OK: JSON salvo em stok_api_sample.json")
except Exception as e:
    print("ERRO_JSON:", repr(e))
